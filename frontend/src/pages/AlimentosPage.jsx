...
import { 
    ...
    faCheckCircle, faToggleOff, faEyeSlash, faDownload
...
const AlimentosPage = () => {
    ...
    const descargarEtiquetaAlimento = (a) => {
        ...
        doc.setFontSize(12);
        doc.text("MALFI ALIMENTOS", 40, 12, { align: "center" });
        doc.setTextColor(40);
        doc.setFontSize(11);
        doc.text(a.nombre, 40, 40, { align: "center" });
        doc.setFontSize(14);
        doc.text(`$${a.precio_venta}`, 40, 55, { align: "center" });
        doc.setFontSize(9);
        doc.text(`Stock: ${a.stock}`, 40, 68, { align: "center" });
        if (a.vencimiento_alimento) {
            doc.text(`Vence: ${new Date(a.vencimiento_alimento).toLocaleDateString('es-AR')}`, 40, 76, { align: "center" });
        }
        doc.setFontSize(8);
        doc.text("--------------------------------", 40, 85, { align: "center" });
        doc.text("Gracias por elegir Malfi", 40, 93, { align: "center" });
        doc.save(`Etiqueta_${a.nombre}.pdf`);
    };

    const exportarExcel = async () => {
        await exportarExcelEstilizado({
            data: alimentos.map(a => ({
                nombre: a.nombre,
            ...
            filename: 'Stock_Alimentos_Malfi.xlsx',
            sheetName: 'Alimentos',
            headerColor: 'D35400'
        });
    };
    ...
    return (
        <div className="container-fluid min-vh-100 p-4 position-relative" style={{
                ...
                <div className="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
                    ...
                    <div className="d-flex gap-2">
                        <button className="btn btn-danger rounded-pill px-4 shadow-lg text-white fw-bold" onClick={() => { cargarPapelera(); setShowPapelera(true); }} style={{ backgroundColor: '#D82F43', border: '2px solid rgba(255,255,255,0.4)' }}>
                            <FontAwesomeIcon icon={faEyeSlash} className="me-2" /> Inactivos
                        </button>
                                    ...
                                    <div className={`card border-0 shadow-sm p-3 rounded-4 h-100 transition-card ${infoVenc.vencido ? 'opacity-75' : ''}`}
                                        ...
                                        <div className="d-flex justify-content-between align-items-center mt-auto">
                                            ...
                                            <div className="d-flex gap-1">
                                                <button className="btn btn-sm btn-outline-info rounded-circle bg-white" title="Descargar etiqueta" onClick={e => { e.stopPropagation(); descargarEtiquetaAlimento(a); }}><FontAwesomeIcon icon={faDownload} /></button>
                                                ...
                                                <button className="btn btn-sm btn-outline-danger rounded-circle bg-white" title="Desactivar" onClick={e => { e.stopPropagation(); setIdToDelete(a.id); setShowConfirm(true); }}><FontAwesomeIcon icon={faToggleOff} /></button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
            ...
            <ConfirmModal
                ...
                title="¿Desactivar este alimento?" message="Se moverá a Inactivos y podrás restaurarlo más tarde."
                ...
                <div className="modal d-block" style={{ backgroundColor: 'rgba(0,0,0,0.6)', zIndex: 2000 }}>
                    <div className="modal-dialog modal-dialog-centered">
                        <div className="modal-content border-0 rounded-4 shadow-lg p-4 text-dark">
                            ...
                            <button className="btn btn-info w-100 rounded-pill mt-4 fw-bold shadow-sm text-white" onClick={() => descargarEtiquetaAlimento(itemSeleccionado)}>
                                <FontAwesomeIcon icon={faDownload} className="me-2" /> Descargar Etiqueta
                            </button>
                            <button className="btn btn-success w-100 rounded-pill mt-2 fw-bold shadow-sm" onClick={() => setShowDetalle(false)}>CERRAR</button>
                        </div>
                    </div>
                </div>
            )}
