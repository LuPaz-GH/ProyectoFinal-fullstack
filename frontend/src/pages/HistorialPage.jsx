...
import { 
    ...
    faChevronLeft, faChevronRight, faDownload, faToggleOff, faEyeSlash
        ...
        if (mascotaSeleccionada) {
            await exportarExcelEstilizado({
                data: historial.map(h => ({
                    fecha: h.fecha_formateada,
                    diagnostico: h.diagnostico,
                    tratamiento: h.tratamiento,
                    peso: formatearPeso(h.peso)
                })),
                columns: [
                    { header: 'Fecha', key: 'fecha', width: 18 },
                    { header: 'Diagnóstico', key: 'diagnostico', width: 35 },
                    { header: 'Tratamiento', key: 'tratamiento', width: 35 },
                    { header: 'Peso', key: 'peso', width: 12 },
                ],
                filename: `Historial_${mascotaSeleccionada.nombre}.xlsx`,
                sheetName: 'Historial',
                headerColor: '2980B9'
            });
        } else {
            await exportarExcelEstilizado({
                data: mascotas.map(m => ({
                    mascota: m.nombre,
                    dueno: m.dueno_nombre
                })),
                columns: [
                    { header: 'Mascota', key: 'mascota', width: 22 },
                    { header: 'Dueño', key: 'dueno', width: 26 },
                ],
                filename: 'Mascotas_Malfi.xlsx',
                sheetName: 'Mascotas',
                headerColor: '2980B9'
            });
        }
                            ...
                            {paginatedMascotas.map(m => (
                                <div className="col-md-4 col-sm-6" key={m.id}>
                                    <div className="card border-0 shadow-lg p-4 rounded-4 position-relative hover-shadow" style={{ background: 'rgba(255, 255, 255, 0.75)', backdropFilter: 'blur(12px)', cursor:'pointer' }} onClick={() => seleccionarMascota(m)}>
                                        ...
                                        <button className="btn btn-outline-danger position-absolute top-0 end-0 m-3 rounded-circle shadow-sm" title="Desactivar" style={{ background: 'white' }} onClick={(e) => { e.stopPropagation(); setIdToDelete(m.id); setDeleteType('mascota'); setShowConfirm(true); }}><FontAwesomeIcon icon={faToggleOff} /></button>
                                    </div>
                                </div>
                            ))}
                        </div>
                                        ...
                                        <tr key={reg.id}>
                                            ...
                                            <td className="text-center">
                                                <div className="d-flex gap-2 justify-content-center">
                                                    ...
                                                    <button className="btn btn-sm text-danger" onClick={() => { setIdToDelete(reg.id); setDeleteType('historial'); setShowConfirm(true); }} title="Desactivar"><FontAwesomeIcon icon={faToggleOff}/></button>
                                                </div>
                                            </td>
                                        </tr>
                                    ))}
