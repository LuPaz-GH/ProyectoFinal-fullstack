...
const DueñoDashboard = () => {
    ...
    const exportarExcel = async () => {
        ...
        await exportarExcelEstilizado({
            data: [
                { concepto: 'Venta Hoy', monto: reporte.totales?.dia || 0 },
                { concepto: 'Esta Semana', monto: reporte.totales?.semana || 0 },
                { concepto: 'Mes Actual', monto: reporte.totales?.mes || 0 },
                { concepto: 'Venta Año', monto: reporte.totales?.anio || 0 },
            ],
            columns: [
                { header: 'Concepto', key: 'concepto', width: 22 },
                { header: 'Monto ($)', key: 'monto', width: 16 },
            ],
            filename: `Reporte_Malfi_${new Date().toLocaleDateString().replace(/\//g, '-')}.xlsx`,
