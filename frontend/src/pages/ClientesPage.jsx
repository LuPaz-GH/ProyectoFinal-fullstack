...
import { 
  ...
  faFilePdf, faFileExcel, faTrashCan, faToggleOff, faEyeSlash
...
const ClientesPage = () => {
  ...
  const exportarExcel = async () => {
    await exportarExcelEstilizado({
      ...
      columns: [
        { header: 'Dueño', key: 'nombre', width: 25 },
        { header: 'DNI', key: 'dni', width: 14 },
        { header: 'Teléfono', key: 'telefono', width: 16 },
        { header: 'Dirección', key: 'direccion', width: 28 },
        { header: 'Mascotas', key: 'mascotas', width: 30 },
      ],
      filename: 'Listado_Clientes_Malfi.xlsx',
      sheetName: 'Clientes',
      headerColor: '1A6FC4'
    });
  };
        ...
        <header className="d-flex justify-content-between align-items-center mb-5 flex-wrap gap-3">
          ...
          <div className="d-flex gap-2 align-items-center">
              ...
              <button className="btn btn-warning rounded-pill px-3 fw-bold shadow-lg text-white" onClick={() => { cargarPapelera(); setShowPapelera(true); }} style={{ border: '2px solid rgba(255,255,255,0.4)' }}>
                <FontAwesomeIcon icon={faEyeSlash} className="me-1" /> INACTIVOS
              </button>
                            ...
                            <div className="card-body p-0 d-flex flex-column text-dark">
                                ...
                                <div className="mt-auto d-flex justify-content-between align-items-center pt-3 border-top">
                                    ...
                                    <div className="d-flex gap-2" onClick={e => e.stopPropagation()}>
                                        ...
                                        <button className="btn btn-sm btn-light rounded-circle shadow-sm" title="Desactivar" onClick={() => { setItemEliminar({id: cliente.id, tipo: 'dueno'}); setShowConfirm(true); }}><FontAwesomeIcon icon={faToggleOff} className="text-danger" /></button>
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            {clientesPaginados.map(c => (
                                <tr key={c.id} style={{ cursor: 'pointer' }} onClick={() => { setClienteSeleccionado(c); setShowDetalle(true); }}>
                                    ...
                                    <td className="text-end pe-4" onClick={e => e.stopPropagation()}>
                                        ...
                                        <button className="btn btn-sm text-danger" title="Desactivar" onClick={() => { setItemEliminar({id: c.id, tipo: 'dueno'}); setShowConfirm(true); }}><FontAwesomeIcon icon={faToggleOff}/></button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
