...
const operacionService = {
    ...
    registrarMovimiento: async (datos) => {
        const { tipo_operacion, categoria, descripcion, monto, metodo_pago, usuario_id, nombre_cliente } = datos;
        const [res] = await pool.query(
            `INSERT INTO caja
             (fecha, tipo_operacion, categoria, descripcion, monto, metodo_pago, usuario_id, nombre_cliente)
             VALUES (NOW(), ?, ?, ?, ?, ?, ?, ?)`,
            [tipo_operacion, categoria, descripcion, monto, metodo_pago, usuario_id, nombre_cliente || null]
        );
        return res.insertId;
    },

    actualizarMovimiento: async (id, datos) => {
        const { descripcion, monto, metodo_pago, nombre_cliente } = datos;
        await pool.query(
            `UPDATE caja
             SET descripcion = ?, monto = ?, metodo_pago = ?, nombre_cliente = ?
             WHERE id = ?`,
            [descripcion, monto, metodo_pago, nombre_cliente || null, id]
        );
    },
