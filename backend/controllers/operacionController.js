...
const operacionController = {
    ...
    getMovimientosCaja: async (req, res) => {
        try {
            const movimientos = await operacionService.getMovimientosCaja(req.query);
    ...
    buscarClientes: async (req, res) => {
        try {
            ...
            const [rows] = await pool.query(
                `SELECT id, nombre, telefono, dni FROM duenos WHERE nombre LIKE ? AND activo = 1 ORDER BY nombre ASC LIMIT 10`,
                [`%${q}%`]
            );
            res.json(rows);
        } catch (err) {
